...
//        We enforce a maximum length of 59 for the passwords, at the UI level
  ...
  public Response userExists(@AuthRequired({Privilege.ADMIN}) User user, @NotNull @PathParam("username") String username) {
  ...
  public Response findByID(@AuthRequired({Privilege.ADMIN}) User user, @NotNull @PathParam("id") long id) {
  ...
  public void deleteUser(@AuthRequired({Privilege.ADMIN}) User admin, @NotNull @PathParam("id") long id) {
